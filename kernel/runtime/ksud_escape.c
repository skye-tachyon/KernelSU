...
__attribute__((cold))
static noinline void sys_execve_escape_ksud_internal(const char __user **filename_user)
{
	// see if its init
...
__attribute__((cold))
static noinline void kernel_execve_escape_ksud_internal(void **filename_ptr)
{
	// see if its init
	if (!is_init(current_cred()))
		return;
